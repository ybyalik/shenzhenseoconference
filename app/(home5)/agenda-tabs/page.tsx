...
/* ──────────────── DAY 1 AS ONE LIST WITH TABS (the trial) ───────────────── */

/** Heading for one half of Day 1, colour-coded so the two programmes stay
 *  distinguishable even when they run together in the All view. */
function SectionHead({ label, accent, blurb }: { label: string; accent: string; blurb: string }) {
  return (
    <div className="pb-4 mb-6 border-b" style={{ borderColor: 'rgba(249, 249, 249, 0.12)' }}>
      <div className="flex items-center gap-2.5">
        ...
        <span
          className="display uppercase text-[15px] md:text-[19px]"
      ...
      <p
        className="mt-2 max-w-[720px] text-[12px] md:text-[13.5px] leading-[168%]"
...
function WorkshopsBlock({ heading = true }: { heading?: boolean }) {
  return (
    <div id="day-1-workshops" className="scroll-mt-[110px]">
      {heading && (
        <SectionHead
          label="Workshops"
          accent="#6FCF9A"
      ...
      <p
        className="mt-7 text-[12px] md:text-[14px] leading-[170%]"
...
function CityToursBlock({ heading = true }: { heading?: boolean }) {
  return (
    <div id="day-1-city-tours" className="scroll-mt-[110px]">
      {heading && (
        <SectionHead
          label="City Tours"
          accent="#5DAEDB"
          blurb="Premium tours blending Shenzhen's high-tech innovation, Lingnan cultural heritage, and coastal vistas, with a focus on executive networking. Curated for international attendees."
        />
      )}
      <CityToursMatrix />
    </div>
...
type ConferenceDay = {
  ...
  /** One line above the tab pills saying what they are for. Two bare words
   *  read as decoration; people skip filters they don't recognise. */
  tabsHint?: React.ReactNode;
...
const CONFERENCE_DAYS: ConferenceDay[] = [
  // The tabbed trial: one card again, but the tab that opens by default shows
  // both programmes in one list. The other two narrow the view rather than
  // being the only way to reach half the day, which is what went wrong before.
  {
    anchor: 'day-1',
    ...
    collapsible: true,
    defaultOpen: true,
    tabsHint: 'Showing both programmes. Narrow the view:',
    tabs: [
      {
        label: 'All',
        content: (
          // mt-10 on the wrapper, not on the blocks: inside the flex column the
          // gap already separates them, and a margin on each would stack on top
          // of it and double the space between the two halves.
    ...
    >
      ...
      {day.tabs && day.tabs.length > 1 && (
        <div className="mt-7">
          {day.tabsHint && (
            <div
              className="mb-2.5 uppercase text-[10px] md:text-[11px]"
              style={{
          ...
          <div
            role="tablist"
            className="inline-flex max-w-full overflow-x-auto"
            style={{
              padding: 8,
              alignItems: 'center',
              borderRadius: 104,
            ...
            {day.tabs.map((t, i) => {
              ...
              return (
                <button
                  ...
                  // The unselected pill used to be bare text on a dark ground,
                  // which read as a label rather than something to press. It
                  // now carries its own surface so all three look like buttons.
                  className={`display uppercase rounded-full px-4 md:px-5 py-2 transition-colors whitespace-nowrap text-[12px] md:text-[14px] ${
                    isActive ? '' : 'hover:bg-white/[0.12]'
                  }`}
                  style={{
                    color: isActive ? '#F9F9F9' : 'rgba(249, 249, 249, 0.72)',
                    fontWeight: 700,
                    lineHeight: 'normal',
                    background: isActive
